...
def call(Map config = [:]) {
    ...
    def repoURL = config.get('REPO_URL', '')
    def branch = config.get('BRANCH', 'main')
    def basePath      = config.get('CODE_BASE_PATH', 'mysql-role')
    def actionMessage = config.get('ACTION_MESSAGE', "Deploying MySQL Database to ${environment}")
        ...
        stages {
            ...
            stage('User Approval') {
                ...
                steps {
                    ...
                    input message: "Approve deployment of MySQL to ${environment}?", ok: "Proceed Deployment"
                }
            }

            stage('Playbook Execution') {
                steps {
                    echo "Executing Ansible Playbook for MySQL Database Setup"
                    dir(basePath) {
